...
class Get_Challenges
{
    ...
    public function __construct()
    {
		...
		$this->EE->db->select('ct.entry_id, ct.entry_date, ct.expiration_date, ct.title, ct.url_title, 
							  cd.field_id_105, cd.field_id_106 as initial_weight, cd.field_id_107 as goal_weight, cd.field_id_130, cd.field_id_131 as chal_photo,  cd.field_id_132 as chal_story, cd.field_id_112 as final_weight,
        ...
        switch ($parameter)
		{
			...
			case "single":
			  ...
			  {				 
			  		...
			  		$pledges[] = array('pl_auth_id' => $row['field_id_116'],                                'pl_end_date' => $row['expiration_date'], 'pl_title' => $row['title'], 'pl_amount' => $row['field_id_119'], 'pl_comment' => $row['field_id_122'], 'pl_pledge_date' => $row['entry_date']);
